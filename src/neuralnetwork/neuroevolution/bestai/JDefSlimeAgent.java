...
public class JDefSlimeAgent extends SlimeAI implements Agent {
    ...
    private static final String SaveFile = "jdef";


    public JDefSlimeAgent() {
...
//        slimeAIs.add(crapAI);
        ...
        slimeAIs.add(new TooSickAI());
//        slimeAIs.add(new BeatsDefenseAgent());
//        slimeAIs.add(new DecentAI());
...
//        	}
            ...
            final int WINNING_WEIGHT = 60;

            if (result.getWinner() == 1) {
            	// weight winning faster.
            	// max is 40 - 1 = 39, min is 40 - 20 = 20 pts
                points += WINNING_WEIGHT - 2.0 * weightNetCrosses(result.getRtlNetCrosses());
            } else if (result.getWinner() == 0) {
                // loss
                points += weightNetCrosses(result.getRtlNetCrosses()) / 2;
            } else {
            	//max is about 20
	...
	public Agent mutate(double mutationRate) {
		...
		JDefSlimeAgent a = new JDefSlimeAgent();
...
//        		normalizeVelocity(p2YV),
    ...
    public static void main(String[] args) {
        ...
        JDefSlimeAgent ai2 = new JDefSlimeAgent();
        ai2.load();
